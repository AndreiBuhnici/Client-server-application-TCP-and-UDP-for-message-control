...
int main(int argc, char *argv[]) {
    ...
    // Enable port reuse, disable nagle, and bind sockets
    int reuse;
        ...
        for(int i = 0; i <= fdmax; i++) {
            if(FD_ISSET(i, &tmp_fds)) {
                if(i == udp_socket) {
                    ...
                    // Receive upd msg and check if there are clients subscribed to topic
                    ...
                    // If clients are on send messages else put them in queue until online
                    ...
                    // Connect new clients if not already connected 
                    ...
                    // Read from stdi, only exit command, which closes server and all clients
                    ...
                    if(ret == 0) {
                        ...
                        // If subscribe message and client not already subscribed, add to topics
                        if (sub_op.type == 's') {
                            struct tcp_message ack_msg;
                            if (topic_cl.find(sub_op.topic) != topic_cl.end()) {
                                if(topic_cl[sub_op.topic].find(sock_clients.at(i)) != topic_cl[sub_op.topic].end()) {
                                    printf("Already subscribed\n");
                                    memset(&ack_msg, 0, sizeof(struct tcp_message));
                                    strcpy(ack_msg.data, "ackf");
                            ...
                            // Send ack
                            memset(&ack_msg, 0, sizeof(struct tcp_message));
                            strcpy(ack_msg.data, "acks");

                            ret = send(i, &ack_msg, sizeof(struct tcp_message), 0);
                            DIE(ret < 0, "Ack sub failed");

                        } else if (sub_op.type == 'u') {
                            // Unsubscribe if subscribed to topic, remove from maps
                            struct tcp_message ack_msg;
                            sub_op.topic[strlen(sub_op.topic) -1] = '\0';
                            if(topic_cl.find(sub_op.topic) != topic_cl.end()) {
                                if(topic_cl[sub_op.topic].find(sock_clients.at(i)) == topic_cl[sub_op.topic].end()) {
